...
const columns: ColumnDef<TransactionHistoryRow>[] = [
  {
    ...
    cell: ({ row }) => (
      <div className="flex gap-2 capitalize">
        {row.original.catagoryIcon}
        <div className="capitalize">{row.original.catagory}</div>
      </div>
    ),
  },
  {
...
function TransactionTable({ from, to }: Props) {
  ...
  const categoriesOptions = useMemo(() => {
    ...
    history.data?.forEach((transaction) => {
      categoriesMap.set(transaction.catagory, {
        value: transaction.catagory,
        label: `${transaction.catagoryIcon} ${transaction.catagory}`,
      });
    });
  ...
  return (
    <div className="w-full">
      ...
      <SkeletonWrapper isLoading={history.isFetching} fullWidth={true}>
